...
- An **OpenClaw 6.6 session** — the runtime. Handles prompt assembly, tool calls, conversation state.
...
**Personality tab** (since v0.1.3):
- Agents → [agent] → Personality tab.
- Rich-text editors for `IDENTITY.md` (personality, voice, domain knowledge) and `SOUL.md` (core beliefs, goals, non-negotiables).
- Changes save to the agent's git branch and auto-enqueue an `update` command. No file-browser navigation needed for the two most commonly edited files.

**In the UI file browser**:
...
Agent detail pages use **slug-based URLs** (`/agents/[slug]`) since v0.1.3. Old UUID-based links auto-redirect to the slug URL, so existing bookmarks and shared links continue to work.
...
Agents can create and be assigned **subtasks** — regular tasks with a `parent_id` pointing to a parent task. Nesting is one level only: a subtask cannot itself have subtasks.

This enables a natural delegation pattern through the org chart:

1. A manager agent receives a complex task.
2. It breaks the task into subtasks and assigns each to a direct report (an agent with `reports_to_id` pointing to the manager).
3. Each subtask assignment triggers an inbox item for the assigned agent via the `enqueue_task_assignment` trigger.
4. As direct reports complete subtasks, the manager tracks progress on the parent task.

Subtasks appear nested under their parent in the task list and on the task detail page. The `task_relations` system tracks the parent/child relationship (`parent_of` / `child_of` relation types). When all subtasks of a parent are marked complete, the parent task surfaces a completion indicator — but it is not auto-closed, since the manager may want to review before marking done.

Humans can also create subtasks manually from the task detail page and assign them to agents or other humans.
...
Since v0.2.0, new agents inherit the gateway's authenticated **default model** instead of a hardcoded per-provider map. This means the model a new agent starts with depends on whatever the operator configured as the default in Settings, rather than a static mapping baked into the codebase. Existing agents are unaffected — their model setting is already persisted.
...
For shared knowledge across agents, use the [knowledge system](/concepts/knowledge) — workspace-scoped items are automatically included in boot context, agent-scoped items appear as an index, and library-scoped items are searchable on demand.
...
The `--reason` parameter on every skill write is surfaced in the UI. It should be a one-line explanation of what changed and why (e.g., "Added YouTube channel research method after 3 successful tasks").
