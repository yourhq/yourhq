...
If you lose the `gateway-state` volume, agents can be reprovisioned from the database, but accumulated browser sessions, local files, and auth tokens are gone. Use `GIT_REMOTE_URL` to continuously back up agent branches to GitHub/Gitea. See [Operations → Backing up](/self-host/operations#backing-up).
