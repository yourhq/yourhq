...
| `embedder` | Python daemon. Eagerly downloads/loads the local BGE model, serves embeddings over an internal HTTP endpoint, and indexes pending knowledge sources into searchable chunks. Documents are the first source type; the schema is ready for files and external sources. | [`gateway/embedder/`](https://github.com/yourhq/yourhq/tree/main/gateway/embedder) | none | `knowledge_sources`, `documents` via indexing lease RPCs | `knowledge_chunks`, `documents.embedding`, indexing status metadata |
