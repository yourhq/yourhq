...
After setup, the workspace can be managed from Settings. Additional workspaces can be added from Settings → Workspaces — the "Add Workspace" flow has full parity with the initial onboarding wizard, including intent selection, infrastructure connection, model provider setup, and first agent creation.

The UI stores workspace metadata in `/config/workspaces.json` and workspace secrets in `/config/secrets.json` on the `ui-config` volume. The active workspace is selected at runtime, so the published UI image does not need a rebuild for each user's Supabase project.
...
- Recurrence rules.
- **Subtasks** — tasks support one-level nesting via `parent_id`. Create subtasks inline from the task detail panel. Parent tasks display a progress chip (done/total) in both list and board views, with a progress bar in the detail panel. Subtask status can be toggled directly from the subtask list. Subtasks inherit the parent's stream and show a breadcrumb link back to the parent task.
...
Knowledge items support folders, tags, rich editing, and semantic + full-text search.
...
Agent boot context is controlled by **scope**:

- A **library-scoped** item is shared workspace-wide context, included in every agent's startup context.
- A **workspace-scoped** item lives in the workspace knowledge base but is not automatically injected into agent context.
...
The gateway stack includes seven services (all behind the `gateway` Compose profile):
...
Gateway state — auth tokens, agent configurations, secrets, and messaging channel pairing (e.g. Telegram) — is automatically backed up to Supabase Storage on shutdown and restored on fresh boot. This means a gateway can be destroyed and recreated without losing its operational state.

Backups are managed from **Settings → Backups**, where you can view existing backups, trigger a manual backup, or restore from a previous snapshot.
...
<CardGroup cols={3}>
  <Card title="API key" icon="key">OpenAI, Anthropic, Gemini, DeepSeek, Mistral, MiniMax, and more.</Card>
