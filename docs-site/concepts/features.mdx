...
HQ is an agent operations system, not just an agent chat UI. It gives one operator a workspace for CRM, tasks, knowledge, routines, collections, and a fleet of long-lived AI agents that run on infrastructure the operator controls.
...
Every HQ install connects to one or more Supabase projects. A project contains the workspace data: agents, tasks, CRM records, knowledge, routines, collections, settings, gateway registrations, command history, and usage data.
...
- Knowledge items (workspace pinned + agent-scoped boot context).
- Agent playbooks — playbooks the agent created or updated autonomously, with recency indicators and edit reasons.
...
- **Playbooks** — structured procedures and SOPs.
