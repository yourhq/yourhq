...
Codespaces users: the installer sets `CODESPACE_NAME` / `GITHUB_CODESPACES_PORT_FORWARDING_DOMAIN` automatically if they're in the environment. If you're running Codespaces locally (VS Code tunnel), set `ALLOWED_ORIGINS` manually. See [Configuration](/reference/configuration) for the full list of origin-related vars.
...
Re-running the installer and picking option 2 (Tailscale) does the same thing. See [Networking](/self-host/networking) for the full networking model.
...
Then update `.env` with the new IP (`HOST_REACHABLE_URL=http://<ip>`) and `docker compose up -d --force-recreate`. See [Networking](/self-host/networking).
...
You send a DM to your agent's bot (Telegram or Discord) or a message in Slack. No reply.

**Why it happens**

For **Telegram and Discord**: 99% of the time you skipped the pairing step. OpenClaw's default policy is `pairing` — the bot won't engage until you've paired your account to the agent.
...
2. Open the UI → the agent's detail page → **Pairing Code** field → paste the code → submit.
3. Send another message. Agent engages.

If the bot doesn't reply with a pairing code on the first message either, it means the bot token is invalid or the runner never ran `add-agent.sh` successfully — see the "provisioning" symptom above.

**How to fix (Slack)**

1. Verify the App-Level Token (`xapp-...`) has the `connections:write` scope.
2. Verify the Bot Token (`xoxb-...`) has the required scopes (`chat:write`, `im:history`, `im:read`, `im:write`).
