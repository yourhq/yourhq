...
If the bot doesn't reply with a pairing code on the first message either, it means the bot token is invalid or the runner never ran `add-agent.sh` successfully — see the "provisioning" symptom above. See [Agents](/concepts/agents).
