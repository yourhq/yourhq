...
- **RLS is tenant-scoped.** Every table enforces RLS via `tenant_id` — users
  can only access rows belonging to their tenant. Self-hosted installs use a
  single default tenant, so all authenticated users share the workspace. Make
  your Supabase signup **invite-only** (Supabase dashboard → Authentication →
  Sign up: disabled, then invite by email). Otherwise anyone who can create a
  Supabase account in your project gets full access to the default tenant's data.
...
UI Supabase config lives in the workspace registry, not `.env`. Open the UI,
go to Settings → Database, and verify the active workspace's URL and keys
