...
For `task_assignment` inbox items, the dispatcher queries `task_relations` to check for unresolved `blocked_by` dependencies. If the assigned task has active blockers (blocking tasks not in `done` or `cancelled` status), the dispatcher appends a note to the inbox summary:

```
⚠ This task has N unresolved blocker(s). Check dependencies before starting.
```
