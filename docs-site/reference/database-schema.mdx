...
The `_schema_version` table tracks the current schema version (currently at v27). The UI reads `EXPECTED_SCHEMA_VERSION` and compares it to the applied version to show update banners when migrations are pending.
...
The schema version table currently tracks v27.
