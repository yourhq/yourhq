...
| `038_gateway_backups.sql` | Gateway backup storage bucket, backup metadata columns on `gateways`, `backup_gateway` command action |
| `039_knowledge_library_scope.sql` | `library` scope for knowledge items, `pinned` column removal and migration |

For a fresh Supabase project, the onboarding wizard handles schema installation automatically with a one-click install. You can also run all files in filename order from the SQL Editor manually.
...
- `gateways` — registered gateway hosts, heartbeat state, labels, reachable URLs, and backup metadata (`last_backup_at`, `last_backup_size_bytes`).
...
- `tasks` — operator and agent work items. Supports `model_override` and `thinking_override` columns for per-task model selection when assigned to an agent. Status enum includes `missed` for overdue escalation. Tasks support subtasks via the `parent_id` column — tasks with `parent_id` set are subtasks of the referenced parent task. The main task list filters on `parent_id IS NULL` to show only top-level tasks; subtasks are displayed inline under their parent.
...
- `knowledge_items` — unified knowledge store for pages, skills, files, and sources. Supports scope (`workspace`, `agent`, or `library`), folders, tags, embeddings, and full-text search.
...
Boot context is controlled by scope:

- A **workspace-scoped** item is injected in full into every agent's startup context.
- An **agent-scoped** item assigned to specific agents via `knowledge_item_agents` is index-only in those agents' boot context and fetched on demand.
- A **library-scoped** item is not included in boot context. It remains searchable and fetchable on demand.
...
The `emit_plugin_event()` function is a generic trigger function that writes to the event queue and strips sensitive fields (e.g. `encrypted_value`). It's attached to 11 triggers across core tables.

See [Plugins](/concepts/plugins) for the full architecture and [Plugin events](/reference/plugin-events) for the complete event reference.
