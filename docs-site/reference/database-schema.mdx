...
- `source_connections` — external source integrations (Notion, Google Drive) with credentials and sync schedules. References `secrets` via `secret_id` FK for encrypted OAuth token storage.
