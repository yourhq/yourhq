...
<Tabs>
  ...
  <Tab title="OAuth / interactive providers">
    ...
    | Provider | Notes |
    |---|---|
    | **OpenAI Codex** | OAuth flow — HQ shows a URL, you complete login in browser |
...
Each agent can use a different provider and model. This lets you run a cheap local model for a background researcher while a cofounder agent uses GPT-4o.

To override for one agent: open the agent's detail page → look for the **Model** selector in the right rail → pick provider and model.
