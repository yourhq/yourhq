...
Exec=/usr/bin/google-chrome-stable --no-sandbox --disable-dev-shm-usage --user-data-dir=$HOME/.openclaw/browser/${BROWSER_PROFILE}/user-data --remote-debugging-port=${CDP_PORT} --remote-allow-origins=http://localhost --no-first-run --no-default-browser-check
Icon=google-chrome
Terminal=false
SHORTCUT
