    ...
    def on_event(self, event: PluginEvent) -> PluginResponse | None:
        if event.event_type == "budget.exceeded":
            ...
            self.ctx.logger.warning(
                f"Agent {agent_id} exceeded budget: ${spent:.2f} / ${limit:.2f}"
            )
            return PluginResponse(
                log_message=f"Agent {agent_id} exceeded budget (${spent:.2f}/${limit:.2f})"
            )
        ...
        if event.event_type == "usage.recorded":
            ...
            self.ctx.logger.warning(
                f"Agent {agent_id} at {pct:.0f}% of budget (${spent_usd:.2f}/${limit_usd:.2f})"
            )
            return PluginResponse(
                log_message=f"Agent {agent_id} at {pct:.0f}% of budget"
            )
